...
import type { DataTableFilterMeta, DataTableFilterEvent } from 'primereact/datatable';
...
export interface UsuarioDTO {
  ...
  email: string;
  name?: string;
  roles?: string[];   // viene desde la API
  rolesStr?: string;  // derivado para filtrado global
...
export default function UsuariosClient() {
  ...
  const fetchUsuarios = async () => {
    try {
      ...
      // Normaliza roles -> rolesStr (string) para filtrado/orden global
      const normalized = (data ?? []).map((u) => ({
        ...u,
        rolesStr: Array.isArray(u.roles) ? u.roles.join(', ') : (u as any).roles ?? ''
      }));
      ...
      <div className="col-12">
        <div className="card">
          ...
          <DataTable
            ...
            globalFilterFields={['name', 'email', 'rolesStr']}
          >
            ...
            <Column
              field="rolesStr" // usamos el string para sort/filter; mostramos con body el array
              header="Privilegios"
              sortable
              body={(row: UsuarioDTO) => row.roles?.join(', ') ?? '—'}
              headerStyle={{ minWidth: '14rem' }}
            />
            {/* Si quieres mostrar la fecha: */}
            {/* <Column header="Creado" body={formatFecha} sortable headerStyle={{ minWidth: '14rem' }} /> */}
