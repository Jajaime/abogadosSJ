...
export function buildMutuoAcuerdo(ctx, caseInfo) {
  return createClauseResult({
    ...
    sections: {
      ...
      hechos: [
        `Se solicita el pago de los feriados legales y proporcionales pendientes al momento del término de la relación laboral, se solicita el pago Feriado Legal de ${ctx?.demandaFmt?.feriado_legal_habiles_text ?? '____________________'} por ${ctx?.demandaFmt?.feriado_legal_monto_clp ?? '____________________'} y Feriado Proporcional de ${ctx?.demandaFmt?.feriado_proporcional_habiles_text ?? '____________________'} por ${ctx?.demandaFmt?.feriado_proporcional_monto_clp ?? '____________________'}.`,
      ],
      derecho: [],
      peticiones: [],
      otrosi: [],
